                            ...
                            if strcmp(lastwarn, '')
                                if matrix_dimensions(1) > 0 && matrix_dimensions(2) > 0
                                    ...
                                    if count == msg_length
                                        ...
                                        %received_matrix = reshape(readvalues, matrix_dimensions)
                                    end
                                end
                            end
...
global MINIEXPLORER
if ~isempty(MINIEXPLORER.serial_text)
    disp( char(MINIEXPLORER.serial_text) );
...
global MINIEXPLORER
MINIEXPLORER.serial_text = [MINIEXPLORER.serial_text text];
end
